...
var MonitorPid = function (pid, options) {
  ...
  self.doMonitoring = function (cb) {
    ...
    if (self.options.fakePidStat) {
      parsePidStatStdout(
        ...
        fs.readFileSync(__dirname + '/../test/big-pidstat-stdout.txt', 'utf8'),
        cb
      );
    } else {
      self.getPidTree(self.pid, function (err, pids) {
        ...
        var pidstat = exec('LANG=C pidstat -d -u -r -p ' +
                            pids.join(',') + ' 1 1',
                            function (err, stdout, stderr) {
                              if (err) return self.emit('error', err);
                              parsePidStatStdout(pids, stdout, cb);
                            });
      });
    }
  ...
  function parsePidStatStdout(pids, stdout, cb) {
    ...
    var record = {
      ...
      monit_time: new Date() - self.hitStartTime,
      ...
      block.split('\n').forEach(function (line, lineIdx) {
        ...
        line.split(new RegExp('[ |\t]+')).forEach(function (field, fieldIdx) {
          ...
          if (lineIdx == 0) {
            ...
            record[fieldsName[fieldIdx]] += parseFloat(field.replace(',', '.'));
          }
        });
      });
    });
  ...
  self.getPidTree = function (pid, cb) {
    ...
    exec('pstree -l -p ' + pid, function (err, stdout, stderr) {
      // ignore threads
      stdout.match(new RegExp('[^}]\\([0-9]+\\)', 'gm')).forEach(function (elt) {
        elt = elt.slice(2, -1);
        if (elt.match(/^[0-9]+$/)) {
